package org.stepik.task.task03.task0309;
...
public class Main {
    ...
    public static class Thief implements MailService {
        private int minPricePackage;
        private int totalSumPackages;

        public Thief(int minPricePackage) {
            this.minPricePackage = minPricePackage;
        }

        public int getStolenValue() {
            return totalSumPackages;
        }
        ...
        public Sendable processMail(Sendable mail) {
            if(mail.getClass() == MailPackage.class) {
                ...
                if(pac.getPrice() >= minPricePackage){
                    totalSumPackages += pac.getPrice();
    ...
    /**
     4) Inspector – Инспектор, который следит за запрещенными и украденными посылками и бьет тревогу в виде исключения, если была обнаружена подобная посылка.
    ...
    public static class Inspector implements MailService{
        ...
        public Sendable processMail(Sendable mail) {
            if (mail instanceof MailPackage) {
                ...
                String lowCaseStringContent = pack.getContent().toLowerCase();
                if (lowCaseStringContent.contains("stones")) {
                    throw new StolenPackageException();
                } else if (lowCaseStringContent.contains(WEAPONS) || lowCaseStringContent.contains(BANNED_SUBSTANCE)) {
